...
目标是先交付一致的英文单语言产品界面，同时把高频文案集中管理；在确定需要多语言前，不急于引入完整翻译框架。按现有静态盘点，工作涉及约 30 处 GUI/状态文案、94 处终端输出、381 处中文日志调用，以及异常文本、配置说明和 LLM 角色名称。粗略工作量为 6～11 人日，不含完整多语言框架和模型实机回归。
...
- [ ] 合并默认润色角色为 `Rewrite`；将固定中英翻译改成可选择目标语言的 `Translate`；将大小助理合并为一个 `Ask AI` 入口。
...
- [ ] 以 `config_client.py`/`config_server.py` 的版本为单一来源，自动同步发布文档。
