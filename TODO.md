...
- [ ] 审查只写 debug、用户表面“没反应”的失败路径，为可操作错误提供托盘、Toast 或终端提示。
...
- [ ] 迁移托盘右键菜单、对话框、按钮、Toast、录音状态和错误提示，并检查固定宽度、换行、字体、DPI 与多显示器布局。
...
- [ ] 检查主屏/副屏、不同 DPI、管理员窗口和无终端打包版；确保 Toast 与录音指示器不重叠。
...
- [ ] 完成 ESC/Toast 取消链路；停止单次输出不得误伤其他请求或全局 Provider。
- [ ] 默认使用完整结果粘贴或 Toast 预览，避免流式模拟键入期间焦点切换把内容写入错误窗口。
