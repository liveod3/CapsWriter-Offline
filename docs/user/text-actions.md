...
“待插入片段”指**本次转写的完整纠正结果**，包括无需修改的部分，不是只返回改动词。参考文字只辅助纠错和首尾标点；即使与本次转写重复，也不应据此删除本次说出的内容。

如果结果异常缩短，先比较终端中的转写与 LLM 结果。诊断中的 `input_chars` / `output_chars` 可确认缩短是否发生在 LLM 返回阶段；若 LLM 结果完整但输入框不完整，再检查文本写入。服务端明确报告达到生成上限时会保留原文；模型正常结束但漏掉内容则不能靠结束状态发现。可临时将纠错预设的 `use_caret_context` 设为 `false` 对照效果，这只关闭 LLM 参考，不关闭 ASR 参考。预设修改在下一次请求生效。
