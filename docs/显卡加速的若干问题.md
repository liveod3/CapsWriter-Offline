...
### Qwen3-ASR 与 Forced Aligner 的加载边界

Qwen3-ASR 和 Qwen3-ForcedAligner 是两套独立模型，分别拥有自己的 ONNX
音频编码器、GGUF 解码器、上下文和运行时缓存：

| 使用场景 | 模型 | 主要输出 |
|------|------|------|
| 麦克风听写、文件粗识别 | Qwen3-ASR | 文本 |
| 文件字幕增强 | Qwen3-ForcedAligner | 文本对应的字词时间戳 |

服务端启动时，ASR 在识别子进程中常驻；Forced Aligner 的兄弟进程只空载
等待，并不会立即加载模型。文件片段需要精确时间戳时，ASR 通过有界队列把
音频和已识别文本交给 Aligner，Aligner 才在自己的进程中按需加载模型并返回
中性的字词时间戳结构。

Aligner 达到 `aligner_idle_timeout` 后退出整个独立进程，由主进程补位一个新的
空载进程。这一点不同于旧版本在 ASR 进程内删除 Aligner 对象：进程隔离可以让
Windows 完整回收 Aligner 的 DirectML/Vulkan、上下文和显存，同时不触碰常驻
ASR 的原生 GPU 运行时。如果 Aligner 崩溃或超过 `aligner_request_timeout`，本次
文件片段会跳过精确时间戳，ASR 不会永久等待。
...
小显存单卡上，短空闲时间可以让 Aligner 尽快归还显存；内存充足但显存较少的
机器也可考虑将 `ForceAlignerGGUFArgs` 的 `onnx_provider` 设为 `CPU`、
`llm_use_gpu` 设为 `False`，再把空闲时间设为 `0` 让 CPU Aligner 常驻。修改这些
配置后必须重启服务端。
...
编辑根目录实际配置 `config_server.py`（字段参考：[服务端默认模板](../config_templates/config_server_template.py)），找到你使用的引擎配置类，修改 `onnx_provider`：
