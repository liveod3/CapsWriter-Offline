...
def refresh_devices(backend) -> bool:
    ...
    if not version.startswith('0.5.'):
        logger.warning('Device refresh unavailable for sounddevice version %s', version)
        return False
    ...
    if not callable(terminate) or not callable(initialize):
        logger.warning('Device refresh API unavailable')
        return False
    initialized = getattr(backend, '_initialized', None)
    if type(initialized) is not int or initialized not in (0, 1):
        logger.warning('Device refresh skipped for unknown or shared initialization state')
        return False
    try:
        ...
        logger.warning('Device refresh failed: %s', type(exc).__name__)
        return False
